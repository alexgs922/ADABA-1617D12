...
public class WarehouseController extends AbstractController {
	...
	public ModelAndView listMyWarehouses() {
		...
		ModelAndView res;
		Collection<Warehouse> warehouses;
		final Distributor d = this.distributorService.findByPrincipal();
	...
	public ModelAndView listOrders(@RequestParam final int warehouseId) {
		...
		final Warehouse w = this.warehouseService.findOne(warehouseId);
		final Distributor principal = this.distributorService.findByPrincipal();
	...
	public ModelAndView display(@RequestParam final int orderId) {
		ModelAndView result;
		final OrderDomain order = this.orderService.findOne(orderId);

		final Collection<Product> cp = order.getProducts();

		result = new ModelAndView("warehouse/displayOrder");
		result.addObject("products", cp);
		result.addObject("requestURI", "warehouse/displayOrder.do?orderId=" + orderId);
