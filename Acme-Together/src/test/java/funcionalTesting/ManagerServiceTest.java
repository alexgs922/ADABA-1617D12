...
public class ManagerServiceTest extends AbstractTest {
	...
	protected void template1(final String username, final int principalId, final String name,final String warehouseAddress, final Class<?> expected) {
		...
		try {
			...
			Warehouse w = this.warehouseService.create();
			w.setName(name);
			w.setWarehouseAddress(warehouseAddress);
			BindingResult binding = null;
			Warehouse res = this.warehouseService.reconstruct(w, binding);
	...
	//Test positivo
		...
		protected void template3(final String username,final String companyAddress,final String companyName,final String email,
				final String vatNumber,final String name,final String password,final String passwordCheck,final String phone,final String webPage,
			...
			try {
				...
				this.authenticate(username);
				 ...
				 DistributorForm customerForm = new DistributorForm();
				...
				Distributor d = this.distributorService.reconstruct(customerForm);
...
public void driver3() {
	...
	final Object testingData[][] = {			
			...
			"admin", "adress","companyName","email@gmail.com","ESA12385678","name","123456","123456",
			"+34955613568","http://www.google.com","surname","managerPrueba",true, null
		}, {
			//Admin intenta registrar en el sistema con email invalido
			"admin", "adress","companyName","email","ESA12385678","name","123456","123456",
			"+34955613568","http://www.google.com","surname","managerPrueba",true, DataIntegrityViolationException.class
		}, {
			//Admin intenta registrar en el sistema con vatNumber invalido
			"admin", "adress","companyName","email@gmail.com","1245678","name","123456","123456",
			"+34955613568","http://www.google.com","surname","managerPrueba",true, DataIntegrityViolationException.class
		}, {
			//Admin intenta registrar manager con contraseñas diferentes
			"admin", "adress","companyName","email@gmail.com","ESA12385678","name","789456","123456",
			"+34955613568","http://www.google.com","surname","managerPrueba",true, DataIntegrityViolationException.class
		}, {
			//Usuario no registrado se registra en el sistema con campos null
			//Un admin registra un manager
			"admin", "adress","companyName",null,null,"name","123456","123456",
			"+34955613568","http://www.google.com","surname",null,true, DataIntegrityViolationException.class	
		},{
			//Usuario no registrado se registra en el sistema sin aceptar las condiciones de uso
			"admin", "adress","companyName","email@gmail.com","ESA12385678","name","123456","123456",
			"+34955613568","http://www.google.com","surname","managerPrueba",false,  DataIntegrityViolationException.class	
		
		}
	};

	for (int i = 0; i < testingData.length; i++)
		this.template3((String) testingData[i][0], (String) testingData[i][1],
				(String) testingData[i][2],(String) testingData[i][3],(String) testingData[i][4],(String) testingData[i][5],
				(String) testingData[i][6],(String) testingData[i][7],(String) testingData[i][8],(String) testingData[i][9],
				(String) testingData[i][10],(String) testingData[i][11],(boolean) testingData[i][12],(Class<?>) testingData[i][13]);
